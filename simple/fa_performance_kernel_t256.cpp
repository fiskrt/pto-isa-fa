...
// TILE_S1=256-specific kernel source. CMake compiles this file only for the t256
// build so short-context optimizations can diverge from the larger-tile kernels.
#define UF_ENABLE 1
...
template <typename PPipe, typename PVPipe, int HEAD_SIZE, int CUBE_S0, int CUBE_S1, int TILE_S1,
...
template <typename QKPipe, typename PPipe, int HEAD_SIZE, int CUBE_S0, int CUBE_S1, int TILE_S1,
                             ...
                             PVecSlotGlobal &pVecSlotGlobal, uint64_t pTileEventId, int blk_idx)
{
    ...
    if constexpr (DAV_VEC) {
        ...
        __gm__ half *p_ptr = p_tile_fifo + qk_base_elems + row_offset * static_cast<size_t>(Tile_S1);
        PStoreGlobal pStoreGlobal(p_ptr);
...
template <typename PVPipe, int HEAD_SIZE, int CUBE_S0, int TILE_S1, int PV_CV_FIFO,
                              ...
                              __gm__ uint8_t *cv_comm_buf, __gm__ uint8_t *profile_buf)
{
    ...
    // Runtime warmup depth (see header). Callers validate the range; the kernel trusts it.
    ...
    // qkPreloadNum is runtime now; bounds are validated on the host in tfa_run.
    ...
    //
    // Short B>1 / Nq>1 shapes have many small work items. A greedy longest-processing-time (LPT)
    // scheduler balances causal triangles well, but its per-item argmin scans every launched core on
    // every core. At 1k-4k contexts that scalar bookkeeping is no longer negligible. Use an O(1)
    // owner mapping for the uniform non-causal rectangle and for many-problem causal launches; rotate
    // causal row-block ranks by problem id so heavy diagonal blocks are spread across cores. Keep LPT
    // for small causal launches, where the exact balance is more important than scheduler overhead.
    //
    // Emission stays problem-major (all of one (batch, q_head)'s row-blocks before the next) so the
    // cores running concurrently stay on one problem's K/V in L2.
    // ---------------------------------------------------------------------------------------------
    const uint32_t full_tiles = S1 / Tile_S1;
    const uint32_t num_problems = batch * num_q_heads;
    const bool use_fast_owner = (!CAUSAL_MASK) || (num_problems >= 16U);
    ...
    const uint32_t fast_start = static_cast<uint32_t>(block_idx);
    const uint32_t work_iters =
        use_fast_owner ?
            (total_row_blocks > fast_start ? 1U + (total_row_blocks - 1U - fast_start) / n_cores : 0U) :
            total_row_blocks;
    for (uint32_t work_iter = 0; work_iter < work_iters; ++work_iter) {
        const uint32_t flat_work =
            use_fast_owner ? fast_start + work_iter * n_cores : work_iter;
        const uint32_t bh = flat_work / block_rows;
        const uint32_t rank_key = flat_work - bh * block_rows;
        uint32_t rank_desc = rank_key;
        if (use_fast_owner) {
            if constexpr (CAUSAL_MASK) {
                rank_desc = (rank_key + block_rows - (bh % block_rows)) % block_rows;
            }
        }
        const int logical_block = static_cast<int>(block_rows - 1U - rank_desc);
    ...
    for (int tile_id = 0; tile_id < num_tiles_s1; ++tile_id) {
        ...
        if constexpr (DAV_VEC) {
            compute_gu<PVPipe, HEAD_SIZE, CUBE_S0, Tile_S1, pv_tile_fifo_size, CV_FIFO_CONS_SYNC_PERIOD,
                       INTERMEDIATE_CHECK, CAUSAL_MASK>(
...
extern "C" __global__ AICORE void tfa_kernel_t256(
...
extern "C" __global__ AICORE void tfa_kernel_causal_t256(
    uint32_t S0, uint32_t S1, uint32_t qk_preload, uint32_t batch, uint32_t num_q_heads, uint32_t num_kv_heads,
    int64_t q_batch_stride, int64_t q_head_stride, int64_t q_seq_stride, int64_t kv_batch_stride,
    int64_t kv_head_stride, int64_t kv_seq_stride, __gm__ uint64_t *ffts_addr, __gm__ half *q, __gm__ half *k,
    __gm__ half *v, __gm__ half *p_tile_fifo, __gm__ float *exp_max_ififo, __gm__ float *global_sum_out,
               ...
               aclrtStream stream, uint8_t *cv_comm_buf)
{
    ...
    if constexpr (CAUSAL_MASK) {
        tfa_kernel_causal_t256<<<n_cores, nullptr, stream>>>(
            S0, S1, qk_preload, batch, num_q_heads, num_kv_heads, q_batch_stride, q_head_stride, q_seq_stride,
        ...
        tfa_kernel_t256<<<n_cores, nullptr, stream>>>(
            S0, S1, qk_preload, batch, num_q_heads, num_kv_heads, q_batch_stride, q_head_stride, q_seq_stride,
            kv_batch_stride, kv_head_stride, kv_seq_stride, (__gm__ uint64_t *)ffts, (half *)q, (half *)k, (half *)v,
            (half *)p_tile_fifo, exp_max_ififo, global_sum_out, exp_max_out, o_out, o_parts_out, qk_tile_fifo,
            pv_tile_fifo, cv_comm_buf, profile_data);
