...
- `tfa_torch_launch.cpp` — C-ABI `tfa_run(q,k,v,o,stream)`; allocates scratch, calls `LaunchTFA<...>`.
- `tfa_poc.py` — creates Q/K/V/O as NPU tensors, launches via `ctypes`, checks vs a torch reference.
...
```
Expect `[poc] PASS` (max abs diff ~2e-4 vs torch reference, atol 1e-3).
